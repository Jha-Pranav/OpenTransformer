...
class GPT2(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                wte=nn.Embedding(
                    ...
                    padding_idx=2,  # make sure to update padding_idx based on the tokenizer #TODO : Correct id for gpt2
                ),
                wpe=nn.Embedding(
                    config.block_size,
                    config.n_embd,
        ...
        self.lm_head = nn.Linear(
            config.n_embd, config.vocab_size, bias=False, device=config.device
        )
    ...
    def from_pretrained(
        ...
        model_type,
        override_args=None,
        cache_dir="multiformer/model_checkpoints/external/",
    ):
